...
io.on('connection', (socket) => {
    ...
    socket.on('startGame', (roomId) => {
        ...
        // 1. 先把防線退回大廳，清除所有舊 AI，只保留真人玩家
        room.players = room.players.filter(p => !p.isAI);

        // 2. 真人不足 4 人，重新補滿全新的 AI
        ...
        // 3. 【核心修正】徹底將真人和 AI 的卡牌(cards)與背包徹底清空，防止疊加
        ...
        io.to(roomId).emit('gameLog', "🏁 舊帳已清！全新的完美配方爭奪戰正式開賽！");
    });
...
function executeMove(room, player, targetPos, roomId) {
    ...
    let logMsg = `🐾 【${player.name}】走到了 ${tileType}。`;
...
server.listen(PORT, () => console.log(`連線伺服器已更新修復`));
